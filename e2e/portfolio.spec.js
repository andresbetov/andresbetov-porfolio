...
test.describe('Portfolio page', () => {
  ...
  for (const { name, width } of WIDTHS) {
    test(`has no horizontal overflow at ${name} (${width}px)`, async ({
      page,
    }) => {
  ...
  test('all sections exist and nav anchors scroll to them', async ({
    page,
  }) => {
  ...
  test('shows three project cards linking to the right repositories', async ({
    page,
  }) => {
  ...
  test('primary button has a hover state', async ({ page }) => {
    const button = page
      .locator("button:has-text('View Projects'), a:has-text('View Projects')")
      .first();
    await button.hover();
